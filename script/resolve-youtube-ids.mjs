...
// Legge config/project.config.json, genera assets/json/video_index.resolved.json come array di righe
...
const CFG = JSON.parse(await fs.readFile(CFG_PATH, 'utf8')).youtube || {}
const PATHS = JSON.parse(await fs.readFile(CFG_PATH, 'utf8')).paths || {}
...
const ALLOWED_AUTHORS = new Set((CFG.allowedAuthors||[]).map(s=>String(s).toLowerCase().replace(/^https?:\/\/(www\.)?youtube\.com\/@/,'').replace(/^@/,'')))
...
function extractFromWatch(html){
  ...
  const handle=(/twitter:site" content="@([^"]{1,100})"/.exec(html)?.[1])||''
