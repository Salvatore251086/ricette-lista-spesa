...
const HEADERS = {
  'User-Agent': 'Mozilla/5.0 Chrome/120 RLS-Crawler/1.4',
...
function extractRecipeLinksFromHtml(html, baseUrl) {
  ...
  $('a[href]').each((_, a) => {
    ...
    try {
      ...
      if (matchCucchiaio(abs)) links.add(abs)
    } catch {}
  })
  return Array.from(links)
}

async function expandToRecipePages(urls) {
  const pages = new Set()
  for (const u of urls) {
    try {
      if (isXml(u)) {
        ...
        $('url loc').each((_, el) => {
          ...
          if (loc && matchCucchiaio(loc)) pages.add(loc)
        })
        ...
        for (const link of extractRecipeLinksFromHtml(html, u)) pages.add(link)
      }
    } catch {}
  }
  return Array.from(pages).slice(0, 400)
}
