...
const ALLOWED_YT_CHANNELS = ['UCj3NcgJQJz0B2s3AqJ4vMwA','UC3d5qL6Q9sH9PqO0F6d0kbg','UCmS4G0rKQ2F0r2m6y0xMari'];
...
function html(targetSelOrNode, content){
  const el = typeof targetSelOrNode==='string' ? document.querySelector(targetSelOrNode) : targetSelOrNode;
...
function suggestRecipes(recipes, inputText){
  ...
  }).filter(x=>x.matches>0).sort((a,b)=>b.score-a.score).map(x=>x.recipe);
}
...
function bestPriceForIngredient(promos, ingredientName){
  ...
  const cand=promos.promotions.filter(p=> (p.aliases||[]).map(x=>String(x).toLowerCase()).includes(name));
...
async function loadRecipes(){ const base=await loadJSON('assets/json/recipes-it.json'); const user=loadUserRecipes(); return [...(base.recipes||[]), ...user] }
...
async function validateYouTubeOnServer(urlOrId){
  ...
  try{ const r=await fetch(url,{cache:'no-store'}); if(!r.ok) return {ok:false, reason:'HTTP '+r.status}; return await r.json() }catch{ return {ok:false, reason:'network'} }
}
...
async function addNewRecipeFromForm(formEl){
  ...
  if(!title || ingredients.length===0){ alert('Titolo e almeno un ingrediente sono obbligatori'); return }

  let youtubeId='';
  if(youtubeInput){
    const res=await validateYouTubeOnServer(youtubeInput);
    if(res && res.ok && ALLOWED_YT_CHANNELS.includes(res.channelId)) youtubeId=res.id;
    else{
  ...
  const current=loadUserRecipes(); current.push(rec); saveUserRecipes(current);
...
function renderTags(tags){ return (tags||[]).map(t=>`<span class="pill">${esc(t)}</span>`).join(' ') }
...
async function renderRecipesView(){
  ...
  grid.addEventListener('click', ev=>{
    ...
    if(action==='add-list'){
      const r=all.find(x=>x.id===btn.dataset.id); if(!r) return;
...
async function renderListView(){
  ...
  const listEl=$('#list');
  function rowHTML(i){
    const sug=i.suggestion ? `<div class="meta">Offerta migliore: ${esc(i.suggestion.store)} · ${esc(i.suggestion.product)} · €${Number(i.suggestion.price).toFixed(2)}</div>` : `<div class="meta">Nessuna offerta</div>`;
    return `
...
function renderAddRecipeView(){
  ...
  html(app, `
    ...
    <form id="form-recipe" style="display:grid;gap:10px;max-width:640px">
      ...
      <p style="font-size:12px;color:#555">La validazione canali YouTube funziona quando l’app è su Cloudflare Pages.</p>
    </form>
  `);
    ...
    <div class="grid">
      ${smart.map(i=>`
        <article class="card">
          <div class="body">
            <div><strong>${esc(i.name)}</strong> · ${Number(i.qty).toString()} ${esc(i.unit||'')}</div>
            ${i.suggestion ? `<div class="meta">Vai da <strong>${esc(i.suggestion.store)}</strong> · ${esc(i.suggestion.product)} · €${Number(i.suggestion.price).toFixed(2)}</div>` : `<div class="meta">Nessuna offerta</div>`}
          </div>
        </article>
      `).join('')}
    </div>
...
function wireNav(){
  ...
  $('#nav-recipes')?.addEventListener('click', ()=>renderRecipesView());
  $('#nav-lista')?.addEventListener('click', ()=>renderListView());
  $('#nav-spesa')?.addEventListener('click', ()=>renderSmartView());
}
